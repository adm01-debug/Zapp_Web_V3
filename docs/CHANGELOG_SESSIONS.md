...
Testado ao vivo: `vitest run src/__tests__/sprint1-security-hardening.test.ts` → 13/13 passando após o fix da assertiva; query em `evo.evolution_messages`/`zapp.contacts` confirmou zero resíduo de teste da rodada 4 (só o lixo pré-existente da rodada antiga, parcialmente limpo).
