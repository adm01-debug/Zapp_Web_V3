...
| 2 | Autenticação | ×3 | 7.0 | **7.0** | MFA TOTP + passkeys existem no código, mas **0 fatores enrolados** (`auth.mfa_factors`); GoTrue sem `GOTRUE_PASSWORD_MIN_LENGTH` (default 6 — a policy de 8+complexidade é só do frontend); JWT 8 h; ✔ refresh rotation, signup off, lockout na 5ª falha (fail-closed) |
...
| 4 | Banco de Dados | ×2 | 7.5 | **7.5** | 522 índices nunca usados (26 MB); drift repo×banco: **8 versões só no banco** (3 de 20/08, 3 contaminação de 30/08 revertida, 2 sicoob 02/09) e **12 só no repo** (11 `20260817260xxx` + `20260903210000`, que está **aplicada mas não registrada** em `schema_migrations`); 97 colunas `status` text vs 2 enum (107 tabelas cobertas por CHECK); 18 `timestamp without time zone`; 244 tabelas vazias; ✔ 0 FK sem índice, 0 tabela sem PK, 0 índice inválido, 0 dead tuples >10 k |
| 5 | CI/CD | ×1 | 6.5 | **7.0** | ✔ main 100 % verde (3 gates vermelhos de 02/09 zerados); ✔ lint-staged bloqueante (#1509). Ainda: typecheck global advisory (`quality-gate.yml:107-115`), `Quality Gate` **não é required check** (só 6 contexts do `ci.yml`), coverage advisory, `bun.lock` com **14 chaves duplicadas** (`bun install --frozen-lockfile` falha; o job "lockfile" admite que não roda frozen e usa heurística de diff), sem rollback automatizado, sem staging |
...
| 8 | Infra / DevOps | ×1 | 8.0 | **6.0** | **Disco 98 %** (seção 1.2); retenção R2 ainda em `DRY_RUN_FIRST_CYCLES=1`; ✔ backup diário + pgBackRest + restore test automatizado 7/7 PASS (04/09); ✔ secrets em Docker secrets; ✔ `CapabilityDrop ALL` + `NoNewPrivileges` no GoTrue; ✔ WAL slots com lag 719 kB |
...
| 15 | Qualidade de Código | ×1 | 7.0 | **7.5** | ✔ lint-staged bloqueante; ✔ 4 `console.log` em src; ✔ 0 `@ts-ignore`; ✔ 0 secrets no código; gaps: `no-explicit-any` = warn (error só em 5 arquivos), **`commit-msg` hook ausente** com commitlint configurado, edge functions fora do ESLint, `tsconfig.json` raiz com `strict` desligado (inofensivo por `files: []`, mas contraditório) |
| 16 | Segurança | ×3 | 7.5 | **7.5** | ✔ headers medidos ao vivo (CSP, HSTS 2 a + preload, SAMEORIGIN, nosniff); ✔ 0 alertas Dependabot, 0 secret-scanning, CodeQL verde; ✔ DOMPurify em 100 % dos `dangerouslySetInnerHTML`; gaps: `script-src 'unsafe-inline' 'unsafe-eval'`; `evolution-webhook` **fail-open** se nenhum secret configurado (`index.ts:52-54`); 10 functions `ai-*`/`csat-auto-send` sem auth (só rate-limit in-memory); `gmail-webhook` tem OIDC no código mas o vault só tem `gmail_pubsub_token` (fallback querystring ativo); 3 CORS wildcard; sem magic bytes em `secure-upload` |
...
5. **`evolution-webhook` fail-closed**: retornar 503 quando nenhum secret estiver configurado (hoje `validateWebhook=null` pula a auth) e desligar `ALLOW_SHARED_SECRET` por default · `supabase/functions/evolution-webhook/index.ts:40-54`
6. **Auth nas 10 functions `ai-*`/`csat-auto-send`**: `requireUser` ou `requireServiceRoleOrCron` (helper já existe em `_shared/auth.ts`) · edge
...
10. **Unificar docs de estado**: `ER_DIAGRAM.md` regenerado do catálogo (ou apontar para `DICIONARIO-BANCO.md`), `CHANGELOG` 2.5.0, deduplicar `INCIDENT-RUNBOOK` · `docs/`
...
**6,8/10.** Em 3 dias o time fechou 5 dos 10 itens do plano anterior (CI verde, testes vermelhos,
lint-staged, paginação realtime, canal WhatsApp de volta) — a máquina de correção funciona. O
que segura a nota é o mesmo diagnóstico de 02/09, agora com um segundo exemplo: **controles que
detectam mas não agem**. O disco chegou a 98 % sob três watchdogs de disco, assim como o WhatsApp
ficou 8,7 dias parado sob vinte. A rota para o 8 é ligar detecção → ação automática (disk-actioner,
fail-closed nos webhooks, gates obrigatórios no merge) e fechar o ciclo repo↔banco das migrations.
