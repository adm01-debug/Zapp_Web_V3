...
test('aceita baseline inalterado ou mais restritivo', () => {
  assert.deepEqual(assertDataLayerBaselineMonotonic(calls(), calls()), {
    baseTotal: 666,
    candidateTotal: 666,
  });
  assert.deepEqual(assertDataLayerBaselineMonotonic(calls(), calls({ 'src/features': 295 })), {
    baseTotal: 666,
    candidateTotal: 665,
  });
});

test('permite redistribuição entre escopos flexíveis sem crescimento global', () => {
...
test('rejeita aumento em escopo rígido mesmo quando o total global diminui', () => {
  const base = calls({
    'src/components': 1,
    'src/features': 200,
    'src/hooks': 300,
...
test('rejeita candidato obsoleto depois que a main ganha teto mais restritivo', () => {
...
test('rejeita totais fora do intervalo de inteiro seguro', () => {
  const unsafe = calls({
...
test('rejeita contadores malformados, ausentes, extras, negativos e fracionários', () => {
  assert.throws(() => parseDataLayerBaseline('{', 'bad'), /JSON inválido/);
  assert.throws(() => parseDataLayerBaseline('{}', 'bad'), /campo calls inválido/);
  assert.throws(
    () => parseDataLayerBaseline(JSON.stringify({ calls: { ...calls(), extra: 0 } }), 'bad'),
    /escopos devem ser exatamente/
  );
  assert.throws(
    () => parseDataLayerBaseline(JSON.stringify({ calls: calls({ 'src/hooks': -1 }) }), 'bad'),
    /contador inválido em src\/hooks/
  );
  assert.throws(
    () => parseDataLayerBaseline(JSON.stringify({ calls: calls({ 'src/hooks': 1.5 }) }), 'bad'),
    /contador inválido em src\/hooks/
  );
});
