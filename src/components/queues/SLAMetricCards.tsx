          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={cn('text-3xl font-bold', getRateColor(data.overallRate))}>
                  {data.overallRate.toFixed(1)}%
                </p>
              </div>
              <div className={cn('rounded-full p-3', getRateBg(data.overallRate))}>
                <Target className={cn('h-6 w-6', getRateColor(data.overallRate))} />
              </div>
            </div>
            <div className="mt-3 flex items-center gap-3">
              <Progress value={data.overallRate} className="h-2 flex-1" />
              {sparkOverall.length >= 2 && (
                <Sparkline data={sparkOverall} width={64} height={20} color="hsl(var(--primary))" />
              )}
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={cn('text-3xl font-bold', getRateColor(data.firstResponse.rate))}>
                  {data.firstResponse.rate.toFixed(1)}%
                </p>
              </div>
            ...
            <div className="mt-3 flex items-center gap-3">
              <div className="flex flex-1 gap-4 text-sm">
                <span className="flex items-center gap-1 text-success">
                  ...
                  {data.firstResponse.onTime} no prazo
                </span>
                <span className="flex items-center gap-1 text-destructive">
                  <XCircle className="h-4 w-4" />
                  {data.firstResponse.breached} atrasados
                </span>
              </div>
              {sparkFR.length >= 2 && (
                <Sparkline data={sparkFR} width={64} height={20} color="hsl(var(--info))" />
              )}
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={cn('text-3xl font-bold', getRateColor(data.resolution.rate))}>
                  {data.resolution.rate.toFixed(1)}%
                </p>
              </div>
            ...
            <div className="mt-3 flex items-center gap-3">
              <div className="flex flex-1 gap-4 text-sm">
                <span className="flex items-center gap-1 text-success">
                  <CheckCircle2 className="h-4 w-4" />
                  {data.resolution.onTime} no prazo
                </span>
                <span className="flex items-center gap-1 text-destructive">
                  <XCircle className="h-4 w-4" />
                  {data.resolution.breached} atrasados
                </span>
              </div>
              {sparkRes.length >= 2 && (
                <Sparkline data={sparkRes} width={64} height={20} color="hsl(var(--primary))" />
              )}
            </div>
          </CardContent>
        </Card>
      ...
      >
        <Card className="border-l-4 border-l-warning">
          <CardContent className="pt-6">
            ...
            <div className="mt-3 flex items-center gap-3">
              <p className="flex-1 text-sm text-muted-foreground">{periodLabel}</p>
              {sparkConversations.length >= 2 && (
                <Sparkline
                  data={sparkConversations}
                  width={64}
                  height={20}
