...
import {
  ...
  getSentimentColor,
  getSentimentBg,
} from '@/hooks/dashboard/useSentimentData';
...
export function SentimentAlertsDashboard() {
  ...
  const { alerts, analyses, loading, stats, dailyData, agentData, fetchData } =
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Monitore o sentimento dos clientes e alertas automáticos
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="outline" className="cursor-not-allowed gap-2 opacity-50" disabled>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          },
          {
            ...
            value: (
              <span className={getSentimentColor(stats.avgSentiment)}>{stats.avgSentiment}%</span>
            ),
            icon: Activity,
          ...
          {
            title: 'Taxa de Negativos',
            value: (
              <span className={stats.negativeRate > 30 ? 'text-destructive' : 'text-success'}>
                {stats.negativeRate}%
              </span>
            ),
            icon: TrendingDown,
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="agents" className="mt-6">
          <AgentsTab agentData={agentData} />
        </TabsContent>
        <TabsContent value="alerts" className="mt-6">
          <AlertsTab alerts={alerts} />
        </TabsContent>
        <TabsContent value="distribution" className="mt-6">
          <DistributionTab stats={stats} analyses={analyses} />
        </TabsContent>
      </Tabs>
    </div>
  );
