...
export const AchievementsSystem = ({ userId, showCompact = false }: AchievementsSystemProps) => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      const unlockedXP = MOCK_ACHIEVEMENTS.filter((a) => a.isUnlocked).reduce(
  ...
  if (showCompact) {
    ...
    const nearComplete = achievements
      .filter((a) => !a.isUnlocked && a.progress / a.target >= 0.7)
      .slice(0, 2);
    return (
      <Card>
          ...
          <div className="mb-3 flex gap-2">
            {recentAchievements.map((a) => {
              ...
              return (
                <div
                  key={a.id}
                  className={`rounded-lg border p-2 ${rarityConfig[a.rarity].border} relative bg-muted/50`}
                >
            ...
            <div className="space-y-2">
              ...
              {nearComplete.map((a) => (
                <div key={a.id} className="flex items-center gap-2">
                  ...
                  <Progress value={(a.progress / a.target) * 100} className="h-1 w-16" />
                </div>
              ))}
            </div>
          )}
                      ...
                      {categoryAchievements.map((achievement, index) => {
                        ...
                        return (
                          <motion.button
                            ...
                            className={`relative rounded-lg border p-3 text-left transition-all ${achievement.isUnlocked ? `${rarity.border} hover:shadow-md` : 'border-border opacity-60 hover:opacity-80'}`}
                          >
                            {achievement.isNew && (
                              <span className="absolute -right-1 -top-1 rounded-full bg-destructive px-2 py-0.5 text-[10px] text-primary-foreground">
                                NOVO
                              </span>
                            )}
                            <div className="flex items-start gap-3">
                              <div
                                className={`rounded-lg p-2 ${achievement.isUnlocked ? rarity.color + '/20' : 'bg-muted'}`}
                              >
                                {achievement.isUnlocked ? (
                                  <Icon
                                    className={`h-6 w-6 ${rarity.color.replace('bg-', 'text-')}`}
                                  />
                                ) : (
