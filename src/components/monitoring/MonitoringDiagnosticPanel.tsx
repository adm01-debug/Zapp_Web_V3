...
function ScoreGauge({ score }: { score: number }) {
  const color =
    score >= 80 ? 'text-primary' : score >= 50 ? 'text-warning-foreground' : 'text-destructive';
  const progressColor =
    score >= 80
      ? '[&>div]:bg-primary'
      : score >= 50
...
function exportDiagnostic(diagnostic: DiagnosticResult) {
  ...
  diagnostic.diagnostics.forEach((d) => {
    ...
    if (d.webhook) {
      ...
      if (d.webhook.missingCritical?.length)
        lines.push(`  Ausentes: ${d.webhook.missingCritical.join(', ')}`);
    }
    if (d.messageFlow) {
      lines.push(
        `  Fluxo: ↓${d.messageFlow.lastHour.incoming} ↑${d.messageFlow.lastHour.outgoing} (${d.messageFlow.flowHealth})`
      );
    }
...
export function MonitoringDiagnosticPanel({
  diagnostic,
  diagnosing,
  onRunDiagnostic,
  ...
  const buildChecklist = (): CheckItem[] => {
    ...
    diagnostic.diagnostics.forEach((d) => {
      items.push({
        ...
        detail:
          d.connectionState === 'open' ? 'Conectado e operacional' : `Estado: ${d.connectionState}`,
      });
      ...
      if (d.webhook) {
        items.push({
          ...
          action: !d.webhook.urlCorrect
            ? { label: 'Corrigir', onClick: () => onReconfigureWebhook(d.instance) }
            : undefined,
        });
        ...
        items.push({
          ...
          detail:
            missing === 0
              ? `${d.webhook.eventsCount} eventos configurados`
              : `${missing} eventos críticos ausentes`,
          action:
            missing > 0
              ? { label: 'Reconfigurar', onClick: () => onReconfigureWebhook(d.instance) }
              : undefined,
        });
      }
      ...
      if (d.messageFlow) {
        items.push({
          ...
          status:
            d.messageFlow.flowHealth === 'healthy'
              ? 'ok'
              : d.messageFlow.flowHealth === 'outbound-only'
                ? 'warning'
                : 'error',
          detail:
            d.messageFlow.flowHealth === 'healthy'
              ...
              : d.messageFlow.flowHealth === 'outbound-only'
                ? 'Apenas enviando, sem recebimento'
                : 'Sem tráfego na última hora',
        });
      }
    });
        ...
        {diagnostic && (
          <div className="ml-auto flex items-center gap-2">
            <Button
              ...
              onClick={() => exportDiagnostic(diagnostic)}
            >
                  ...
                  <Card>
                    <CardHeader className="pb-3">
                      <CardTitle className="flex items-center gap-2 text-base">
                        {d.instance}
                        <Badge
                          variant={d.connectionState === 'open' ? 'default' : 'destructive'}
                          className="text-xs"
                        >
                          {d.connectionState}
                        </Badge>
                    ...
                    <CardContent className="space-y-3">
                      <div className={cn('flex items-center gap-2 rounded-lg p-2.5', sev.bg)}>
                        <SevIcon className={cn('h-4 w-4', sev.color)} />
                        <span className="text-sm font-medium">Webhook: {sev.label}</span>
                        {d.webhookIssue && (
                          <span className="text-xs text-muted-foreground">— {d.webhookIssue}</span>
                        )}
                      </div>
                      {d.autoFix && (
                        <div
              ...
              <CardContent>
                <div className="space-y-2">
                  {checklist.map((item, i) => {
                    const statusIcons = {
                      ok: <CheckCircle2 className="h-4 w-4 shrink-0 text-primary" />,
                      warning: (
      ...
      {!diagnostic && !diagnosing && (
        <Card>
          <CardContent className="py-12 text-center">
            <Stethoscope className="mx-auto mb-3 h-12 w-12 text-muted-foreground/20" />
            <p className="text-sm text-muted-foreground">
              Execute um diagnóstico para verificar o estado completo do sistema.
            </p>
          </CardContent>
        </Card>
      )}
