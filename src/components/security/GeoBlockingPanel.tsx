...
export function GeoBlockingPanel() {
  ...
  const getModeDescription = () =>
    settings?.mode === 'whitelist'
      ? 'Apenas países na whitelist podem acessar'
      : settings?.mode === 'blacklist'
        ? 'Países na blacklist são bloqueados'
        : 'Todos os países podem acessar';
  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="rounded-lg bg-primary/10 p-2">{getModeIcon()}</div>
              <div>
          ...
          {settings?.mode === 'whitelist' && (
            <div className="mt-4 rounded-lg border border-success/20 bg-success/10 p-3">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-sm font-medium text-success">Modo Whitelist Ativo</p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    Apenas usuários de países na whitelist ({allowedCountries.length}) podem acessar
                    o sistema.
                  </p>
                </div>
              </div>
          ...
          {settings?.mode === 'blacklist' && (
            <div className="mt-4 rounded-lg border border-destructive/20 bg-destructive/10 p-3">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-sm font-medium text-destructive">Modo Blacklist Ativo</p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    Usuários de países bloqueados ({blockedCountries.length}) não podem acessar o
                    sistema.
                  </p>
                </div>
              </div>
                ...
                <DialogFooter>
                  ...
                  <Button
                    onClick={() => {
                      ...
                      if (c) handleAddCountry(c.code, c.name);
                    }}
          ...
          <Tabs
            ...
            onValueChange={(v) =>
              setActiveTab(
                v as
                  | 'whitelist'
                  | 'blacklist' /* ignore-audit: Select/Tabs value string narrowed to union; developer controls option values */
              )
            }
          >
                  ...
                  <AnimatePresence mode="popLayout">
                    {filteredCountries.map((country) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`rounded-lg p-2 ${activeTab === 'whitelist' ? 'bg-success/10' : 'bg-destructive/10'}`}
                          >
                            <MapPin
                              className={`h-4 w-4 ${activeTab === 'whitelist' ? 'text-success' : 'text-destructive'}`}
                            />
                          </div>
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              Adicionado{' '}
                              {formatDistanceToNow(new Date(country.created_at), {
                                addSuffix: true,
                                locale: ptBR,
                              })}
                            </p>
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => !open && setCountryToRemove(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Remover {countryToRemove?.country_name}?</AlertDialogTitle>
            <AlertDialogDescription>
              Esta ação removerá o país da {activeTab}. Você pode adicioná-lo novamente a qualquer
              momento.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
