          ...
          <CardContent className="p-0">
            {configs.length === 0 ? (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="flex flex-col items-center justify-center px-4 py-12 text-muted-foreground"
              >
                <AlertTriangle className="mb-3 h-10 w-10 opacity-30" />
                <p className="text-sm font-medium">Nenhuma configuração de SLA</p>
                <p className="mt-1 text-xs opacity-70">
                  Defina metas de tempo de resposta e resolução
                </p>
              </motion.div>
            ) : (
                      ...
                      >
                        <Switch
                          ...
                          onCheckedChange={(checked) =>
                            toggleMutation.mutate({ id: cfg.id, is_active: checked })
                          }
                        />
                          ...
                          <div className="mt-1 flex items-center gap-4 text-xs text-muted-foreground">
                            <span className="flex items-center gap-1">
                              ...
                              <span className="font-medium text-foreground/80">
                                {formatSLAMinutes(cfg.first_response_minutes)}
                              </span>
                            </span>
                            <span className="flex items-center gap-1">
                              <Target className="h-3 w-3" /> Resolução:{' '}
                              <span className="font-medium text-foreground/80">
                                {formatSLAMinutes(cfg.resolution_minutes)}
                              </span>
                            </span>
                          </div>
                        </div>
                        <Button
                          aria-label="Editar configuração de SLA"
                          size="icon"
                          variant="ghost"
                        ...
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button
                              aria-label="Excluir configuração de SLA"
                              size="icon"
                              variant="ghost"
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                Tem certeza que deseja excluir <strong>"{cfg.name}"</strong>? Esta
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      first_response_minutes: parseInt(e.target.value) || 1,
                    }))
                  }
                  className="mt-1"
                />
