    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        {analysis.department && departmentConfig[analysis.department] && (
          <Badge
            variant="outline"
            className={`${departmentConfig[analysis.department].color} text-[10px] font-semibold`}
          >
        ...
        {statusConfig[analysis.status] && (
          <Badge
            variant="outline"
            className={`${statusConfig[analysis.status].className} text-[10px]`}
          >
        ...
        {analysis.urgency && urgencyConfig[analysis.urgency] && (
          <Badge
            variant="outline"
            className={`${urgencyConfig[analysis.urgency].className} text-[10px]`}
          >
        ...
        {analysis.churnRisk && analysis.churnRisk !== 'low' && (
          <Badge
            variant="outline"
            className={`${churnConfig[analysis.churnRisk]?.color || ''} border-current/30 text-[10px]`}
          >
        ...
        <div className="flex items-center justify-between rounded-xl border border-border/50 bg-muted/20 p-3">
          ...
          <div className="flex items-center gap-0.5">
            {[1, 2, 3, 4, 5].map((star) => (
              <Star
                key={star}
                className={`h-4 w-4 ${star <= (analysis.customerSatisfaction || 0) ? 'fill-warning text-warning' : 'text-muted'}`}
              />
            ))}
