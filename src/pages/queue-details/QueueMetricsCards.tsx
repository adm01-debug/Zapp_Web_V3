...
export function QueueMetricsCards({ metrics }: { metrics: QueueMetrics }) {
  const cards = [
    ...
    },
    {
      icon: Clock,
      label: 'Aguardando',
      value: metrics.waitingContacts,
      color: 'bg-warning/10 text-warning',
    },
    {
      icon: CheckCircle,
      label: 'Resolvidos Hoje',
      value: metrics.resolvedToday ?? 'Sem dados',
      color: 'bg-success/10 text-success',
    },
    {
      icon: TrendingUp,
      label: 'Tempo Médio',
      value: metrics.avgResponseTime ?? 'Sem dados',
        ...
        <Card key={label} className="border border-secondary/20 bg-card/50 backdrop-blur">
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              <div
                className={`flex h-10 w-10 items-center justify-center rounded-lg ${color.split(' ')[0]}`}
              >
